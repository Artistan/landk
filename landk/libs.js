    ...
    Howl.prototype = {
        load: function () {
            ...
            for (var i = 0; i < self._urls.length; i++) {
                var ext = self._urls[i].toLowerCase().match(/.+\.([^?]+)(\?|$)/), canPlay = false;
            ...
            if (self._webAudio) {
                ...
                var listener = function () {
                    ...
                    if (Object.getOwnPropertyNames(self._sprite).length === 0) {
                        self._sprite = {_default: [0, self._duration * 1e3]}
                    }
                    if (!self._loaded) {
                        self._loaded = true;
                        self.on("load")
            ...
            self._inactiveNode(function (node) {
                node._sprite = sprite;
                var pos = node._pos > 0 ? node._pos : self._sprite[sprite][0] / 1e3, duration = self._sprite[sprite][1] / 1e3 - node._pos;
            ...
            var self = this, dist = to, iterations = dist / .01, hold = len / iterations;
            if (!self._loaded) {
                self.on("load", function () {
            ...
            for (var i = 1; i <= iterations; i++) {
                (function () {
                    var vol = Math.round(1e3 * (self._volume + .01 * i)) / 1e3, toVol = to;
                    setTimeout(function () {
                        self.volume(vol);
                        if (vol === toVol) {
            ...
            var self = this, dist = self._volume - to, iterations = dist / .01, hold = len / iterations;
            if (!self._loaded) {
                self.on("play", function () {
    ...
    if (usingWebAudio) {
        ...
        var loadSound = function (obj, buffer) {
            ...
            if (Object.getOwnPropertyNames(obj._sprite).length === 0) {
                obj._sprite = {_default: [0, obj._duration * 1e3]}
            }
            if (!obj._loaded) {
                obj._loaded = true;
                obj.on("load")
...
// formats,re, dispatch, sprintf -- jQuery extensions.
(function ($) {
    var formats = {
        b: function (val) {
